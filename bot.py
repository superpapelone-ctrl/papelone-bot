...
async def check_proattivo(context):
    """Every 2 hours: Claude decides autonomously if there is something important to say."""
    tasks = load_tasks()
    if not tasks:
        return
    ...
    # Only active between 9:00 and 22:00
    ...
    system = f"""Sei il cervello esterno di una persona con ADHD. Ora sono le {now.strftime('%H:%M')} di {day_name} {today.strftime('%d/%m/%Y')}.

Analizza la situazione e decidi IN MODO AUTONOMO se c'è qualcosa di importante da dire adesso.

MANDA UN MESSAGGIO solo se:
- C'è un task urgente con scadenza oggi o domani che probabilmente non ha fatto
- Si avvicina un appuntamento nelle prossime 2-3 ore
- È un momento chiave della giornata (es. ore 15 = pomeriggio produttivo, ore 19 = chiamate famiglia)
- C'è qualcosa che rimanda da giorni e questo è il momento giusto per farlo

NON mandare messaggi se:
- Non c'è niente di urgente
- Ha già ricevuto un reminder recente sullo stesso task
- È fuori dagli orari produttivi

Rispondi SOLO con JSON senza backtick:
{{"send": true, "message": "messaggio breve e diretto in italiano"}}
oppure:
{{"send": false}}
...
Database: {json.dumps(tasks, ensure_ascii=False)}"""
    ...
    try:
        raw = call_claude(
            [{{"role": "user", "content": f"Sono le {{now.strftime('%H:%M')}}. Devo mandare un messaggio proattivo?"}}],
        ...
        if parsed.get("send"):
            ...
            if msg:
                ...
                logger.info(f"Proattivo: messaggio inviato alle {{now.strftime('%H:%M')}}")
        else:
            logger.info(f"Proattivo: nessun messaggio necessario alle {{now.strftime('%H:%M')}}")
    except Exception as e:
...
def main():
    ...
    if job_queue:
        ...
        # Proactive check every 2 hours from 9 to 22
        for hour in [9, 11, 13, 15, 17, 19, 21]:
